...
async def test_task7_c14_composition_supervision_fresh_uuid_and_controlled_close():
    ...
    generic_supervised_watcher_names = (
        ...
        # _session_stall_watcher landed on main after this test was written
        # (interval 30s sleep); the merged GatewayRunner spawns it, so it must
        # be inerted here like the other generic watchers.
        "_session_stall_watcher",
    ...
    with tempfile.TemporaryDirectory(prefix="c14-gateway-") as directory:
        ...
        with ExitStack() as patches:
            ...
            # Upstream main registers bundled platform plugins (e.g. a2a) as
            # deferred loaders and fires them from _resolve_all when gateway
            # startup materializes plugin_entries(). Loading those plugins
            # calls registry.register(), which would bump the frozen tool
            # registry generation and break the hermetic snapshot assertion.
            # The C14 scenario does not exercise platform adapters, so keep
            # the deferred loaders unfired.
            patches.enter_context(
                patch.object(
                    platform_registry_module.platform_registry,
                    "_resolve_all",
                    lambda: None,
                )
            )
