...
export const ja = defineLocale({
  ...
  sidebar: {
    ...
    projects: {
      ...
      menu: 'プロジェクト操作',
      menuRename: '名前を変更',
      menuAcceptCompletion: '完了を承認',
      menuAppearance: '外観',
      menuContinueWork: '作業を続ける',
      noColor: '色なし',
      ...
      activationFailed: 'プロジェクトは作成されましたが、アクティブにできませんでした。',
      createFailed: 'プロジェクトを作成できませんでした',
      mutationConflict: 'プロジェクトの状態が変更されました。最新の状態を確認して再試行してください。',
      mutationFailed: 'プロジェクトを更新できませんでした',
      mutationPending: '再送信する前に、保留中のプロジェクト操作を再試行または解決してください。',
      mutationRetry: '結果を確認できませんでした。同じ操作を安全に再試行してください。',
      runtimeApproval: '承認',
      runtimeApprovalNone: 'なし',
      runtimeApprovalPending: '保留中',
      runtimeBlock: 'ブロック',
      runtimeBlockNone: 'なし',
      runtimeDelivery: '配信',
      runtimeLifecycle: 'ライフサイクル',
      runtimePhase: 'フェーズ',
      runtimeQueue: 'キュー',
      runtimeRetryPending: '再試行待ち',
      runtimeStatus: 'プロジェクト実行状態',
      staleBackend:
  ...
  statusStack: {
    ...
    managedProject: {
      ambiguousSession:
        '複数の管理対象プロジェクトがこのセッションを使用しています。メッセージは送信されませんでした。',
      attachmentsUnsupported: '管理対象プロジェクトの添付ファイルはまだサポートされていません。下書きを復元しました。',
      conflict: 'プロジェクトが変更されました。正規状態を確認して再送信してください。',
      historyUnsupported: '管理対象プロジェクトの履歴変更はまだサポートされていません。',
      invalidRetryReceipt: '管理対象プロジェクトの再試行から無効なキュー追加結果が返されました。',
      legacyDraftRecovery: '以前の下書きを復元',
      messageFailed: '管理対象プロジェクトのメッセージ送信に失敗しました。',
      missingAcceptedTurn: '管理対象プロジェクトのキュー追加から受理済みターンが返されませんでした。',
      restoreDraft: '下書きを復元',
      retryFailed: '管理対象プロジェクトの再試行に失敗しました。',
      retryRequired: '正規のキュー追加結果が不確定です。同じリクエストを再試行してください。',
      retryScopeChanged: '管理対象プロジェクトの再試行範囲が変更されました。',
      retryUnavailable: '管理対象プロジェクトの再試行は利用できません。',
      stopUnavailable: '管理対象プロジェクトに停止できる実行中のターンがありません。',
      runtimeUnavailable: '管理対象プロジェクトのランタイムはまだ利用できません。',
      voiceDraftRecovery: '音声下書きの復元',
      voiceScopeChanged: '音声入力の文字起こし中にプロジェクトが変更されました。音声下書きを隔離しました。',
      title: 'プロジェクトメッセージ'
    },
    coding: {
      title: 'ワークツリー',
  ...
  assistant: {
    ...
    clarify: {
      managedUnsupported:
        '管理対象プロジェクトでは明確化入力はサポートされていません。Hermes が正規に処理するまで、この実行はブロックされたままです。',
